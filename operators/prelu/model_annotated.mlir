...
module {
  func.func @main(%arg0: tensor<1x64xf32> {secret.secret}) -> tensor<1x64xf32> {
    ...
    %3 = linalg.generic {domain_lower = -2.0 : f64, domain_upper = 3.0 : f64, degree = 3 : i32, indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel"]} ins(%arg0 : tensor<1x64xf32>) outs(%2 : tensor<1x64xi1>) {
    ^bb0(%in: f32, %out: i1):
      %5 = arith.extf %in : f32 to f64
      %6 = arith.cmpf olt, %5, %cst_0 : f64
      linalg.yield %6 : i1
    } -> tensor<1x64xi1>
    %4 = linalg.generic {domain_lower = -2.0 : f64, domain_upper = 3.0 : f64, degree = 3 : i32, indexing_maps = [#map, #map, #map, #map], iterator_types = ["parallel", "parallel"]} ins(%3, %1, %arg0 : tensor<1x64xi1>, tensor<1x64xf32>, tensor<1x64xf32>) outs(%0 : tensor<1x64xf32>) {
