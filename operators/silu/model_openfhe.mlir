...
module attributes {backend.openfhe, scheme.actual_slot_count = 16384 : i64, scheme.ckks, scheme.requested_slot_count = 1024 : i64} {
  func.func @main(%cc: !cc, %arg0: tensor<1x!ct> {tensor_ext.original_type = #original_type}) -> (tensor<1x!ct> {tensor_ext.original_type = #original_type}) {
    ...
    %cst_1 = arith.constant dense<0.48834371566772461> : tensor<1024xf64>
    %cst_2 = arith.constant dense<-0.07166607677936554> : tensor<1024xf64>
    %cst_3 = arith.constant dense<-0.044202778488397598> : tensor<1024xf64>
    %cst_4 = arith.constant dense<2.000000e+00> : tensor<1024xf64>
    %cst_5 = arith.constant dense<1.000000e+00> : tensor<1024xf64>
    %cst_6 = arith.constant dense<0.57044214010238647> : tensor<1024xf64>
    ...
    %ct_35 = openfhe.level_reduce_inplace %cc, %extracted {levelToDrop = 3 : i64} : (!cc, !ct) -> !ct
    %ct_36 = openfhe.mul_plain %cc, %ct_35, %pt_21 : (!cc, !ct, !pt) -> !ct
    %ct_37 = openfhe.mod_reduce_inplace %cc, %ct_36 : (!cc, !ct) -> !ct
    %ct_38 = openfhe.mul_no_relin %cc, %ct_34, %ct_37 : (!cc, !ct, !ct) -> !ct
    %ct_39 = openfhe.relin_inplace %cc, %ct_38 : (!cc, !ct) -> !ct
    %0 = tensor.empty() : tensor<1x!ct>
    %ct_40 = openfhe.mod_reduce_inplace %cc, %ct_39 : (!cc, !ct) -> !ct
    %inserted = tensor.insert %ct_40 into %0[%c0] : tensor<1x!ct>
  ...
  func.func @main__generate_crypto_context() -> !cc {
    %params = openfhe.gen_params  {mulDepth = 5 : i64, plainMod = 0 : i64} : () -> !params
