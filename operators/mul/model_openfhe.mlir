...
module attributes {backend.openfhe, scheme.actual_slot_count = 8192 : i64, scheme.ckks, scheme.requested_slot_count = 1024 : i64} {
  func.func @main(%cc: !cc, %arg0: tensor<1x!ct> {tensor_ext.original_type = #original_type}, %arg1: tensor<1x!ct> {tensor_ext.original_type = #original_type}) -> (tensor<1x!ct> {tensor_ext.original_type = #original_type}) {
    ...
    %ct = openfhe.mul_no_relin %cc, %extracted, %extracted_0 : (!cc, !ct, !ct) -> !ct
    %ct_1 = openfhe.relin_inplace %cc, %ct : (!cc, !ct) -> !ct
    %0 = tensor.empty() : tensor<1x!ct>
    %ct_2 = openfhe.mod_reduce_inplace %cc, %ct_1 : (!cc, !ct) -> !ct
    %inserted = tensor.insert %ct_2 into %0[%c0] : tensor<1x!ct>
